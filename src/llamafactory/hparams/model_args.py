...
class QuantizationArguments:
    ...
    # bitnet
    use_bitnet: bool = field(
        default=False,
        metadata={"help": "Whether or not to use bitnet quantization training."},
    )
    standard_bitnet: bool = field(
        default=False,
        metadata={"help": "Whether or not to use standard_bitnet bitnet."},
    )
    inquant_layers_keys: List[str] = field(
        default_factory=list,
        metadata={"help": "List of layer keys (as strings) to apply bitnet quantization."},
    )
    unload_pretrained_modoule_keys: str = field(
        default='',
        metadata={"help": "Comma-separated keys like '.26.,.27.'"}
    )
    weight_quant_method: str = field(
        default="minmax",
        metadata={"help": "weightquant method: minmax, gpqt, block, perchannel."},
    )


    ## distill for bitnet fintuning
    distill: bool = field(
        default=False,
        metadata={"help": "Whether or not to use distill."},
    )
    teacher_model_path: str = field(
        default=None,
        metadata={"help": "teacher path for distilling."},
    )

    ### logits distill
    beta: float = field(
        default=1.0,
        metadata={"help": "the weight for logits distill loss."},
    )
    logits_distill_temperature: float = field(
        default=5.0,
        metadata={"help": "the temperature for logits distill loss."},
    )

    ### minilm v2
    split_head_number: int = field(
        default=32,
        metadata={"help": "the number of splited heads usedin minilmv2."},
    )
    minilmv2_loss_coeff: float = field(
        default=1000.0,
        metadata={"help": "the weight for minilmv2 loss."},
    )
    layers_used_for_distill: List[int] = field(
        default_factory=list,
        metadata={"help": "List of layer used for distilling."},
    )
    distill_to_student_final_layer: bool = field(
        default=False,
        metadata={"help": "distill the teacher feature to student's final layer or not"},
    )
